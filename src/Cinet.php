 ...
 * @created_at 27/09/2022
 * @last_update 29/09/2022
 * @github https://github.com/Prosper226?tab=repositories
 * @repos https://github.com/Prosper226/cinetPay.git
 * @version 1.0.
 */
...
class Cinet{
    ...
    public function __construct($business = null){
        ...
        $this->ACCESS   = $this::getAccess($business, 'json');
        $this->apiAuth  = $this->ACCESS['Api'];
        $this->endpoint = $this->ACCESS['endpoint'];
        $this->isoCountry   = $this->ACCESS['isoCountry'];
    ...
    private static function getAccess($business = null, $format = 'json'): Array{
        try{
            switch($format){
                case 'json' : 
            ...
            if (!isset($ACCESS[$business])) throw new Exception("$business is not configured");
    ...
    public function deposit($isoCountry = null, $data = ["phone" => null, "amount" => null, "bash" => null]){
        try{
            ...
            if($result["code"] == '201'){
                ...
                return ["code" => 200, "data" => $returned, "payment_url" => $url];
            }else{
                $url = null;
                return ["code" => 500, "message" => $result["code"].' : '.$result["description"] ];
            }
        }catch(Exception $e){
            // throw new Exception('$e->getMessage()');
                ...
                try {
                    ...
                    if(hash_equals($xtoken, $generated_token))
                    {
                        ...
                        $log  = "User: ".$_SERVER['REMOTE_ADDR'].' - '.date("F j, Y, g:i a").PHP_EOL.
                            ...
                            "-------------------------".PHP_EOL;
                        file_put_contents(dirname(__DIR__)."/.logs", $log, FILE_APPEND);
