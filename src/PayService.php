...
class PayService{
    ...
    public function addContact($transfer)
    {
        ...
        $phone = $transfer['phone'];
        $prefix = $transfer['prefix'];
        $name = 'BeneficiaryName : '. $transfer['transfer_id'];
        $surname =  'BeneficiarySurname : ' . $transfer['transfer_id'];

        $params[] = [
            'prefix'    => $phone,
            'phone'     => $prefix,
            'name'      => ($name) ? $name : 'dummydummy',
            'surname'   => ($surname) ? $surname: 'dummydummy',
    ...
    public function checkBalance(string $countryIso = null)
    {
    ...
    public function checkBalanceAll()
    {
        $url = $this->cinetPayApiBaseUrl . '/transfer/check/balance?token=' . $this->getToken() . '&lang=fr';
        try {
            $response = $this->fetch($url);
